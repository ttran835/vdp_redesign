    ...
    // also render single image for carousel
     ...
     foreach ($vehicle->media->image as $image) {
       ...
       if ($rotate === 90 || $rotate === 270) {
         if ($flip === 0) { 
           echo "<div><img class='img-responsive zoom-init' src='{$image->src}' alt='{$vehicle->label}' style='transform: rotate({$rotate}deg) scale(0.6)'></div>";
         } else {
           echo "<div><img class='img-responsive zoom-init' src='{$image->src}' alt='{$vehicle->label}' style='transform: rotate({$rotate}deg) scale(0.6) scaleY(-1)'></div>";
         }
       } else {
         if ($flip === 0) {
           echo "<div><img class='img-responsive zoom-init' src='{$image->src}' alt='{$vehicle->label}' style='transform: rotate({$rotate}deg) scale(1)'></div>";
         } else {
           echo"<div><img class='img-responsive zoom-init' src='{$image->src}' alt='{$vehicle->label}' style='transform: rotate({$rotate}deg) scale(1) scaleX(-1)'></div>";
         }
       }
     }
    ?>
