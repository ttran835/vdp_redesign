  ...
  <div class="col-xs-12">
    ...
    <!-- Nav tabs -->
      <ul id="" class="nav nav-tabs" role="tablist">
        <?php 
          foreach ($vehicle->equipment->standard as $option) {
            $id = strtolower(preg_replace('/\&*\s*\/*-*,*/', '', $option));
            echo '<li role="presentation"><a href="'.$option.'" aria-controlers="'.$option.'" role="tab" data-toggle="tab">'.$option.'</a></li>';
          }
      ...
      <div class="tab-content">
        ...
        <?php 
          foreach ($vehicle->equipment->standard as $option => $group) {
            $id = strtolower(preg_replace('/\&*\s*\/*-*,*/', '', $option));
            echo '<div role="tabpanel" id="#'.$id.'" class="tab-pane fade"></div>';
    ...
    <div class="panel-group" id="standard-specification-accordion">
      <?php 
        foreach ($vehicle->equipment->standard as $option => $group) {
          $id = strtolower(preg_replace('/\&*\s*\/*-*,*/', '', $option));
          echo '<div class="panel pdtm-vdp-specs"></div>';
          echo '<a class="panel-title" data-toggle="collapse" data-parent="#standard-specification-accordion" data-target="'.$id.'">'.$id.'</a>';
          echo '<div class="panel-collapse collapse" id="'.$id.'">';
